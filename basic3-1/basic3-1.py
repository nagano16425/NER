        ...
        if len(test_data[i:i+batch_size]) == batch_size:
            tf_unary_scores, tf_transition_params = sess.run([yreshape, transition_params], 
                                                            feed_dict={x:test_data[i:i+batch_size], 
                                                                       ...
                                                                       keep_prob_out:1.0})
            # CRF viterbi decode
            viterbi_decode = _viterbi_decode(tf_unary_scores, 
                                             test_sent_len[i:i+batch_size], 
...
if __name__ == "__main__":
    ...
    accuracy = tf.reduce_sum(tf.cast(correct_prediction, "float") * tf.nn.embedding_lookup(l_look, sqlength)) / tf.cast(tf.reduce_sum(sqlength), "float")
