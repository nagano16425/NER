...
# Read Hierarchy
def _read_hierarchy(filename):
   f = tf.gfile.GFile(filename, "r")
   hierarchy = []
   hierarchy_tree = []
   for line in f.readlines():
      pair = line.strip()
      hierarchy.append(pair)
      pair_extend = pair.split(" ")
      hierarchy_tree.append(pair_extend)
   return hierarchy
...
def _output(iterations):
   ...
   del ylist[:]
   # Output(Writing iob2)
   with tf.gfile.Open("RNN"+eval_type+"_"+iterations+".iob2", 'w') as iob2:
   # with open("RNN"+eval_type+"_"+iterations+".iob2", 'w', encoding="utf-8") as iob2:
...
if __name__ == "__main__":
   ...
   train_hierarchy_list = _read_hierarchy(hierarchy_path)
   ...
   # Weight & bias
   # W1 = tf.Variable(tf.random_uniform([bi_hidden_size, tag_size], -1.0, 1.0))
   # b1 = tf.Variable(tf.random_uniform([tag_size], -1.0, 1.0))
   # W2 = tf.Variable(tf.random_uniform([bi_hidden_size, tag_size], -1.0, 1.0))
   # b2 = tf.Variable(tf.random_uniform([tag_size], -1.0, 1.0))
   ...
   if tf.train.get_checkpoint_state("./model/"+eval_type+"/"):
      ckpt = tf.train.get_checkpoint_state("./model/"+eval_type+"/")
      for i in range(iterations):
         if (i+1) % (iterations/10) == 0:
            load_path = ("./model/"+eval_type+"/my-model"+eval_type+"-"+str(i+1))
            print("Model load in file: %s" % load_path)
            saver.restore(sess, load_path)
            print ("---output start---")
            # Output(Result for Prediction)
            iob2 = _output(str(i+1))
            print ("---output finish---")
   else:
      print ("---train start---")
      # Training
