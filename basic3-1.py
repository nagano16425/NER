...
Detail:InputSize(200)
       Embedding(100)
       Batch(10)
       BiDirectional-LSTM(1Layer,150cell)
       CRF Layer
       Forget-bias(0.0)
       Adam(Optimizer)
       Dropout(0.8,0.5)
...
# LSTM(BiDirectional)
def _bidirectionalLSTM(word_vec):
   ...
   inputs = [word_vec[:,time_step,:] for time_step in range(input_size)]
   ...
   output = tf.reshape(tf.concat(1, outputs), [-1, bi_hidden_size], name="concat")
   return output, outputs, l

# CRF
def _CRF(inputs, tag_indices, l):
   outputs, transition_params = tf.contrib.crf.crf_log_likelihood(inputs, tag_indices, l)
   return outputs, transition_params
...
def _outputCRF(iterations):
   ylist = []
   ylist_rest = []
   rest = int(len(test_data) % batch_size)
   for i in range(0,len(test_data),batch_size):
      if len(test_data[i:i+batch_size]) == batch_size:
         tf_unary_scores, tf_transition_params = sess.run([yreshape, transition_params],
                                                          feed_dict={x:test_data[i:i+batch_size],
                                                                     y_:test_data[i:i+batch_size],
                                                                     sqlength:test_sent_len[i:i+batch_size],
                                                                     keep_prob_wv:1.0,
                                                                     keep_prob_out:1.0})
         # CRF viterbi decode
         viterbi_decode = _viterbi_decode(tf_unary_scores, test_sent_len[i:i+batch_size], tf_transition_params)
         ylist.extend(viterbi_decode)
   tf_unary_scores, tf_transition_params = sess.run([yreshape, transition_params],
                                                    feed_dict={x:test_data[-batch_size:],
                                                               y_:test_data[-batch_size:],
                                                               sqlength:test_sent_len[-batch_size:],
                                                               keep_prob_wv:1.0,
                                                               keep_prob_out:1.0})
   # CRF viterbi decode
   viterbi_decode_rest = _viterbi_decode(tf_unary_scores, test_sent_len[-batch_size:], tf_transition_params)
   ylist_rest.extend(viterbi_decode_rest)
   ...
   del ylist[:]
   # Output(Writing iob2)
   with open("RNN"+eval_type+"_"+str(iterations)+".iob2", 'w', encoding="utf-8") as iob2:
...
# CRF viterbi decode
def _viterbi_decode(scores, lengths, params):
   viterbi_decode = []
   for viterbi_scores, viterbi_length in zip(scores, lengths):
      viterbi_scores = viterbi_scores[:viterbi_length]
      if viterbi_length != 0:
         viterbi_sequence, _ = tf.contrib.crf.viterbi_decode(viterbi_scores, params)
         viterbi_decode.append(viterbi_sequence)
      else:
         viterbi_decode.append("")
...
if __name__ == "__main__":
   ...
   hierarchy_path = os.path.join(data_path, "ror_hierarchy.txt")
   ...
   # Closs Entropy
   cross_entropy = tf.reduce_sum(tf.reduce_mean(-crf_score)
                                 * tf.nn.embedding_lookup(l_look, sqlength)
                                 / tf.cast(tf.reduce_sum(sqlength), dtype=tf.float32))
   ...
   accuracy = tf.reduce_sum(tf.cast(correct_prediction, "float")
                            * tf.nn.embedding_lookup(l_look, sqlength)) / tf.cast(tf.reduce_sum(sqlength), "float")
   ...
   if tf.train.get_checkpoint_state("./model/"+eval_type+"/"):
      ...
      for i in range(iterations):
         ...
         feed_dict = {x:batch_xs, y_:batch_ys, sqlength:batch_len, keep_prob_wv:0.8, keep_prob_out:0.5}
         sess.run(train_step, feed_dict = feed_dict)
         result = sess.run(merged, feed_dict={x:train_data[0:batch_size],
                                              y_:train_tag[0:batch_size],
                                              sqlength:train_sent_len[0:batch_size],
                                              keep_prob_wv:1.0,
                                              keep_prob_out:1.0})
